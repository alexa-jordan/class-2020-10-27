...
```{r}
treatment_model <- stan_glm(data = week_8,
         att_chg ~ treatment - 1, 
         family = gaussian(), 
         refresh = 0)
```
...
treatment_model %>% 
  as_tibble() %>% 
  pivot_longer(cols = c("treatmentTreated", "treatmentControl"), names_to = "treatment", values_to = "pp") %>% 
  ggplot(aes(x = pp, fill = treatment)) +
  geom_density(aes(y = after_stat(count/sum(count))), alpha = .5, binwidth = 0.05, position = "identity") +
